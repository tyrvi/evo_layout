...
def draw_graph(graph, size=(500, 500)):
    ...
    for v in graph.V:
        circle = plt.Circle((v.pos[0], v.pos[1]), radius=v.radius, fc='y')
        ax.add_patch(circle)
        ax.text(v.pos[0], v.pos[1], str(v.idx), fontsize=12)

    for e in graph.E:
        line = plt.Line2D((e.v1.pos[0], e.v2.pos[0]), (e.v1.pos[1], e.v2.pos[1]), color='black')
        ax.add_line(line)
...
def draw_voronoi(graph, layout=None, lw=2, ps=2):
